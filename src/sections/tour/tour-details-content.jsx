...
function computeAIInsights(tour, bookers, t) {
  const insights = [];
  ...
  const topAssist = [...bookers]
    .filter((b) => b.assists > 0)
  ...
  if (result === 'victory') {
    if (margin >= 3) {
      insights.push({
        color: 'success',
        icon: 'mdi:trophy',
        title: t('label_insight_dominant_win_title'),
        body: `${t('label_insight_dominant_win_body_prefix')} ${margin} ${t('label_insight_dominant_win_body_suffix')}`,
      });
    } else {
      insights.push({
        color: 'success',
        icon: 'mdi:trophy',
        title: t('label_insight_hard_fought_win_title'),
        body: t('label_insight_hard_fought_win_body'),
      });
    }
  } else if (result === 'draw') {
    insights.push({
      color: 'info',
      icon: 'mdi:handshake',
      title: t('label_insight_balanced_match_title'),
      body: t('label_insight_balanced_match_body'),
    });
  } else {
    ...
    insights.push({
      ...
      title: t('label_insight_result_to_analyze_title'),
      body: `${t('label_insight_result_to_analyze_body_prefix')} ${gap}. ${t('label_insight_result_to_analyze_body_suffix')}`,
    });
  }

  if (topScorer?.goals > 0) {
    const assistLine =
      topAssist && topAssist.id !== topScorer.id && topAssist.assists > 0
        ? `, ${t('label_assisted_by')} ${topAssist.name} (${topAssist.assists} ${t('label_assists_abbr')})`
        : '';
    insights.push({
      color: 'primary',
      icon: 'mdi:star-shooting',
      title: t('label_insight_man_of_match_title'),
      body: `${topScorer.name} ${t('label_led_the_offense_with')} ${topScorer.goals} ${topScorer.goals > 1 ? t('word_goals') : t('word_goal')}${assistLine}. ${t('label_decisive_intervention')}`,
    });
  }
  ...
  if (redCards > 0) {
    insights.push({
      ...
      title: t('label_insight_discipline_alert_title'),
      body: `${redCards} ${redCards > 1 ? t('label_ejections_plural') : t('label_ejection_singular')} ${t('label_and')} ${yellowCards} ${yellowCards !== 1 ? t('label_yellow_cards_plural_short') : t('label_yellow_card_singular_short')}. ${t('label_insight_discipline_alert_body_suffix')}`,
    });
  } else if (yellowCards >= 4) {
    insights.push({
      color: 'warning',
      icon: 'mdi:card',
      title: t('label_insight_discipline_risk_title'),
      body: `${yellowCards} ${t('label_insight_discipline_risk_body_suffix')}`,
    });
  } else if (yellowCards === 0) {
    insights.push({
      color: 'success',
      icon: 'mdi:shield-check',
      title: t('label_insight_clean_play_title'),
      body: t('label_insight_clean_play_body'),
    });
  }
  ...
  if (lateCount >= 3) {
    insights.push({
      ...
      title: t('label_insight_punctuality_issue_title'),
      body: `${lateCount} ${t('label_insight_punctuality_issue_body_suffix')}`,
    });
  } else if (lateCount === 0) {
    insights.push({
      color: 'success',
      icon: 'mdi:clock-check',
      title: t('label_insight_committed_team_title'),
      body: t('label_insight_committed_team_body'),
    });
  }
...
export function TourDetailsContent({ tour }) {
  ...
  const aiInsights = useMemo(() => computeAIInsights(tour, bookers, t), [tour, bookers, t]);

  const { home, away } = tour.scores;
  const resultType = home > away ? 'victory' : home === away ? 'draw' : 'loss';
  const resultLabel = {
    victory: t('label_match_result_win'),
    draw: t('label_match_result_draw'),
    loss: t('label_match_result_loss'),
  }[resultType];
    ...
    >
      ...
      <Box sx={{ px: { xs: 3, md: 5 }, pt: 3, pb: 4 }}>
        ...
        >
          ...
          <Box
            sx={{ width: 3, height: 3, borderRadius: '50%', bgcolor: 'rgba(255,255,255,0.2)' }}
          />
          <Stack direction="row" alignItems="center" spacing={0.75}>
            <Iconify
            ...
            <Stack direction="row" alignItems="center" spacing={1.5}>
              <Typography
                sx={{
                  ...
                  fontWeight: 900,
                  lineHeight: 1,
                  textShadow: resultType === 'victory' ? '0 0 40px rgba(34,197,94,0.3)' : 'none',
                }}
              >
                {home}
              </Typography>
        ...
        {tour?.tourGuides?.length > 0 && (
          <Stack direction="row" alignItems="center" justifyContent="center" spacing={1}>
            ...
            <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.4)' }}>
              {t('label_technical_staff_colon')} {tour.tourGuides.map((g) => g.name).join(', ')}
            </Typography>
          </Stack>
        )}
      </Box>
  ...
  const kpis = [
    { label: t('word_goals'), value: totalGoals, icon: 'mdi:soccer', color: '#22C55E' },
    { label: t('label_assists'), value: totalAssists, icon: 'mdi:shoe-cleat', color: '#0C68E9' },
    {
      label: t('word_players'),
      value: bookers.length,
      icon: 'mdi:account-group',
      color: '#61F3F3',
    },
    {
      label: t('label_yellow_cards_plural_short'),
      value: yellowList.length,
      icon: 'mdi:card',
      color: '#FFAB00',
    },
    {
      label: t('label_red_cards_plural_short'),
      value: redList.length,
      icon: 'mdi:card',
      color: '#FF5630',
    },
    {
      label: t('label_kpi_late'),
    ...
    >
      {kpis.map((k) => (
        ...
        >
          ...
          <Typography
            sx={{ fontSize: '1.5rem', fontWeight: 900, lineHeight: 1, color: 'text.primary' }}
          >
            {k.value}
          </Typography>
          <Typography
          ...
          return (
            <Box
              ...
              sx={{
                ...
                border: (theme) => `1px solid ${alpha(theme.palette.grey[500], 0.1)}`,
            ...
            >
              <Stack direction="row" spacing={1} alignItems="flex-start">
                <Iconify
                  icon={ins.icon}
                  width={16}
                  sx={{ color: accent, mt: 0.25, flexShrink: 0 }}
                />
                <Stack spacing={0.25}>
                  <Typography
                    variant="caption"
                    sx={{ fontWeight: 800, color: accent, lineHeight: 1, letterSpacing: 0.3 }}
                  >
  ...
  const renderMVP = mvp && (
    <Card
      sx={{
        p: 2,
        background: (theme) =>
          `linear-gradient(135deg, ${alpha(theme.palette.warning.main, 0.08)} 0%, ${alpha(theme.palette.warning.light, 0.03)} 100%)`,
    ...
    >
      <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1.5 }}>
        ...
        <Typography
          variant="caption"
          sx={{ fontWeight: 800, color: 'warning.dark', letterSpacing: 1 }}
        >
          {t('label_mvp_of_match')}
        </Typography>
      </Stack>
      <Stack direction="row" alignItems="center" spacing={1.5}>
        <Avatar
          ...
          sx={{
            width: 44,
            height: 44,
            border: (theme) => `2px solid ${alpha(theme.palette.warning.main, 0.4)}`,
          }}
        >
          {mvp.name?.charAt(0)}
        </Avatar>
        <Stack spacing={0}>
          ...
          <Typography variant="caption" sx={{ color: 'text.secondary' }}>
            {mvp.goals > 0
              ? `${mvp.goals} ${mvp.goals > 1 ? t('word_goals') : t('word_goal')}`
              : ''}
            {mvp.goals > 0 && mvp.assists > 0 ? ' · ' : ''}
            {mvp.assists > 0 ? `${mvp.assists} ${t('label_assists_abbr')}` : ''}
            {mvp.goals === 0 && mvp.assists === 0 ? t('label_best_performance') : ''}
          </Typography>
        </Stack>
      </Stack>
    </Card>
    ...
    <Card sx={{ p: 2.5 }}>
      ...
      <Stack spacing={2}>
        {[
          {
            list: yellowList,
            label: t('label_yellow_cards_full'),
            icon: 'mdi:card',
            color: '#FFAB00',
          },
          { list: redList, label: t('label_red_cards_full'), icon: 'mdi:card', color: '#FF5630' },
          {
            list: lateList,
            label: t('label_late_arrivals'),
            icon: 'mdi:clock-alert',
            color: '#C684FF',
          },
        ]
    ...
    <Card sx={{ overflow: 'hidden' }}>
      <Button
        ...
        sx={{
          ...
          color: 'text.primary',
          borderRadius: 0,
          '&:hover': { bgcolor: (theme) => alpha(theme.palette.grey[500], 0.06) },
        }}
      >
