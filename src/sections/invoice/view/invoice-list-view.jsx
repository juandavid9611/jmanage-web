...
/**
 * `scopeId` overrides the workspace as the payment-requests "group" — e.g. a
 * tournament id, for the tournament-scoped Pagos page which reuses this same
 * view/table instead of duplicating it. Defaults to the current workspace.
 * `readOnly` hides manual create/edit (payment requests generated by a source
 * like "Generar Cobros" shouldn't be hand-edited outside that flow).
 */
export function InvoiceListView({ scopeId, title, breadcrumbLinks, readOnly = false } = {}) {
  ...
  const groupId = scopeId ?? selectedWorkspace?.id;

  const { paymentRequests } = useGetPaymentRequests(groupId);
  ...
  const handleDeleteRow = useCallback(
    (id) => {
      deletePaymentRequest(id, groupId);
    ...
    [dataInPage.length, table, tableData, groupId]
  );
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <InvoiceTableRow
                        ...
                        onEditRow={readOnly ? undefined : () => handleEditRow(row.id)}
